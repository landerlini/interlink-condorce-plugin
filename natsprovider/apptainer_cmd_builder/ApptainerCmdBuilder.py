...
class ApptainerCmdBuilder(BaseModel, extra="forbid"):
    ...
    description: str = Field(
        default="", description="User-defined description of the job"
    )
    ...
    fuse_sleep_seconds: int = Field(
        default=5, description="Time in seconds between fuse mount and job execution."
    )
    ...
    def volumes(self) -> List[BaseVolume]:
        ...
        return list(
            set(
                [vb.volume for c in all_containers for vb in c.volume_binds]
    ...
    def exec_containers(self):
        ...
        ret += [
            "wait ${pids[%d]}; echo -n $? > %s"
            % (i_container, container.return_code_path)
            for i_container, container in enumerate(self.containers)
        ]

        ret += [
            f"echo '=== Output of container {container.uid} ==='\ncat {container.log_path}"
            for container in (self.init_containers or []) + (self.containers or [])
        ]

        ret += [
            "tar cvf $SANDBOX/logs "
            + " ".join(
                [
                    *[
                        f"-C {c.workdir} {os.path.basename(c.log_path)}"
                        for c in self.init_containers
                    ],
                    *[
                        f"-C {c.workdir} {os.path.basename(c.return_code_path)}"
                        for c in self.init_containers
                    ],
                    *[
                        f"-C {c.workdir} {os.path.basename(c.log_path)}"
    ...
    def cleanup_volume_files(self):
        ret = []
        if any(
            [v.fuse_enabled_on_host and isinstance(v, FuseVolume) for v in self.volumes]
        ):
    ...
    def process_logs(self, tar_file: BinaryIO):
        ...
        for init_container in self.init_containers:
            ...
            init_container.return_code = int(
                ...
                if (os.path.basename(init_container.return_code_path))
                in file_contents.keys()
                else self.error_code_for_return_code_not_found
            )
