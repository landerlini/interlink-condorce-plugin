...
class ContainerSpec(BaseModel, extra="forbid"):
    ...
    def flags(self):
        ...
        if self.tmp_dir_mode == 'bind':
            ...
            ret += [f'--scratch /tmp', f'--scratch /var/tmp', '--scratch /dev', f'--workdir {self.tmp_dir}']
            ...
            ret += [dedent(f"""
                ...
                if [ -e {local_image} ]; then
                    ...
                    if [ -e {cached_image}/md5 ] && [[ "$REMOTE_IMAGE_MD5" == $(cat {cached_image}/md5) ]]; then
                        ...
                        HTTP_STATUS=$(curl -Lo {cached_image}-{rndid}.tar.gz \\
                            -w \"%{{http_code}}\" \\
                            -H \"X-Token: {self.shub_token}\" \\
                            {self.shub_proxy_server}/get-docker/{self.image}) 
                        if [[ $HTTP_STATUS -ge 200 && $HTTP_STATUS -lt 300 ]] \\
                                && [[ "$REMOTE_IMAGE_MD5" == "$(md5sum {cached_image}-{rndid}.tar.gz | cut -d ' ' -f 1)" ]]; then 
