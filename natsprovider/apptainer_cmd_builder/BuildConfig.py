...
class BuildConfig(BaseModel):
    ...
    class SlurmOptions(BaseModel, extra="forbid"):
        ...
        class SlurmFlavor(BaseModel, extra="forbid"):
            ...
            max_resources: Dict[
                Literal["cpu", "memory", "nvidia.com/gpu"], Union[str, int]
            ] = Field(
                default={},
    ...
    class ApptainerOptions(BaseModel, extra="forbid"):
        ...
        fakeroot: bool = Field(
            default_factory=lambda: (
                os.environ.get("APPTAINER_FAKEROOT", "no").lower()
                in ["true", "yes", "y"]
            ),
        ...
        no_home: bool = Field(default=True, description="Do not bind home by default")
        no_privs: bool = Field(
            default=True, description="Drop all privileges from root user in container"
        )
    ...
    class NetworkOptions(BaseModel, extra="forbid"):
        ...
        tap_cidr: str = Field(
            default="172.18.2.0/24", description="Network CIDR for the TAP device"
        )

        tap_mtu: int = Field(
            default=1280, description="Maximum transfer unit of the TAP device"
        )
        ...
        tun2socks_binary: str = Field(
            ...
            description="Local path to tun2socks binary",
        )
    ...
    apptainer: ApptainerOptions = Field(
        default=ApptainerOptions(), description=ApptainerOptions.__doc__
    )
    shub_proxy: SingularityHubProxy = Field(
        default=SingularityHubProxy(), description=SingularityHubProxy.__doc__
    )
    slurm: SlurmOptions = Field(
        default=SlurmOptions(), description=SlurmOptions.__doc__
    )
    resources: Resources = Field(
        default=Resources(),
        description="Computing resources made available by the pool (not by the single submitter!)",
    )
    node: NodeOptions = Field(default=NodeOptions(), description=NodeOptions.__doc__)
    network: NetworkOptions = Field(
        default=NetworkOptions(), description=NetworkOptions.__doc__
    )
    ...
    def __str__(self):
        ...
        for section_name, section in properties.items():
            ...
            if section_description is not None:
                section_description = "\n".join(
                    [f"# {line}" for line in section_description.split("\n") if line]
                )
    ...
    def from_file(cls, filename: Union[str, None]):
        ...
        build_config_file = (
            filename if filename is not None else "/etc/interlink/build.conf"
        )
        ...
        if not os.path.exists(build_config_file):
            if tolerate_missing_file:
                ...
                logger.critical(
                    f"Build configuration file {build_config_file} does not exist."
                )
                exit(MISSING_BUILD_CONFIG_ERROR_CODE)
        else:
            with open(build_config_file, "rb") as input_file:
                return cls(
                    **toml_load(input_file), input_toml_filename=build_config_file
                )
